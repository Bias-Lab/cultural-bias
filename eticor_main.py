...
for persona_index in range(3):
    ...
    for col, data in tqdm(dataset.iterrows(), total=len(dataset), desc="Processing"):
        ...
        for persona in personalities:
            query = prompt_template.create_eticor_prompt(data['region'], data['sentences'], persona_index, persona)
    ...
    if not os.path.exists('e_results'):
        os.makedirs('e_results')
    if not os.path.exists(f'e_results/{model}'):
        os.makedirs(f'e_results/{model}')
        
    dataset.to_csv(f'e_results/{model}/results_{persona_index}.csv', index=False, quoting=csv.QUOTE_NONNUMERIC)
